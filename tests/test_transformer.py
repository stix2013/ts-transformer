...
class TestEncoder:
    def test_encoder_shapes(self):
        ...
        x = tf.random.uniform((batch_size, seq_len), dtype=tf.int32, minval=0, maxval=input_vocab_size)
...
class TestDecoder:
    def test_decoder_shapes(self):
        ...
        decoder = Decoder(num_layers, d_model, num_heads, dff, target_vocab_size, maximum_position_encoding)
        
        batch_size = 64
        seq_len = 10
        
        x = tf.random.uniform((batch_size, seq_len), dtype=tf.int32, minval=0, maxval=target_vocab_size)
        enc_output = tf.random.uniform((batch_size, seq_len, d_model))
        
        output, _ = decoder(x, enc_output, training=False, look_ahead_mask=None, padding_mask=None)
        assert output.shape == (batch_size, seq_len, d_model)
...
class TestTransformer:
    def test_transformer_shapes(self):
        ...
        inp = tf.random.uniform((batch_size, seq_len), dtype=tf.int32, minval=0, maxval=input_vocab_size)
        tar = tf.random.uniform((batch_size, seq_len), dtype=tf.int32, minval=0, maxval=target_vocab_size)
